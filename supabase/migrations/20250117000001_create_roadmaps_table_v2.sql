...
-- Step 4: Database Schema (v2 - Safe Drop & Recreate)
-- =====================================================
...
CREATE FUNCTION get_latest_roadmap(p_user_id UUID)
RETURNS TABLE (
  id UUID,
  roadmap_id TEXT,
...
CREATE FUNCTION get_roadmap_history(p_user_id UUID, p_limit INTEGER DEFAULT 10)
RETURNS TABLE (
  id UUID,
  roadmap_id TEXT,
  roadmap_version TEXT,
...
CREATE FUNCTION roadmap_exists_by_hash(p_user_id UUID, p_input_hash TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
